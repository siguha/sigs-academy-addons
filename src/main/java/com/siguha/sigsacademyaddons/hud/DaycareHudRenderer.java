...
public class DaycareHudRenderer {
    ...
    public void onHudRender(GuiGraphics graphics, DeltaTracker deltaTracker) {
        ...
        if (!hudConfig.isDaycareMenuEnabled()) {
            return;
        }

        if (!daycareManager.hasAnyContent()) {
            return;
        }
        ...
        int panelWidth = calculatePanelWidth(font, displayPens, displayEggs, totalActiveEggs, maxDisplayEggs);
        int panelHeight = calculatePanelHeight(displayPens, displayEggs, totalActiveEggs, maxDisplayEggs);
